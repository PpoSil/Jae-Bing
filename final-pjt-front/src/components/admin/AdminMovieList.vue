      ...
      <div class="row">
        ...
        >
          ...
          <!-- <template #cell(actions)="data">
            <b-button @click="updateMovie(data.item)" class="mr-1 btn btn-warning font-jua"> 수정 </b-button>
            <b-button @click="deleteMovie(data.item)" class="btn btn-secondary font-jua"> 삭제 </b-button>
          </template> -->
        </b-table>
      </div>
    </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      fields: [
        ...
        { key: 'vote_average', label: '총평점', sortable: true, sortDirection: 'desc' },
  ...
  methods: {
    ...
    //     axios
    //       .delete(`${SERVER_URL}/movies/${movie.id}/movie/`)
    //       .then((res) => {
